...
import { MAX_STANDARDS_BUDGET, STANDARDS_AUTO_APPLY_CONFIDENCE, STANDARDS_CONSENSUS_MIN, } from "./config.js";
/**
 ...
 */
export function reviewProposedUpdate(update, reflectionConfidence, agentName, forceApply = false) {
    const proposed = update;
    const db = getDb();
    ...
    // 3. Budget check
    const existing = db.prepare("SELECT content FROM shared_standards WHERE status = 'active'").all();
 ...
 */
function applyUpdate(proposed, agentName, appliedBy) {
    const db = getDb();
    const now = Date.now();
    if (proposed.action === "add") {
    ...
    if (proposed.action === "modify" && proposed.current_text) {
        ...
        const match = db.prepare("SELECT * FROM shared_standards WHERE status = 'active' AND content LIKE ?").get(`%${proposed.current_text.slice(0, 50)}%`);
        if (!match) {
            return { action: "skipped", reason: "could not find matching standard to modify" };
        }
        const lastVersion = db.prepare("SELECT MAX(version) as v FROM standards_history WHERE standard_id = ?").get(match.id);
    ...
    if (proposed.action === "remove") {
        const match = db.prepare("SELECT * FROM shared_standards WHERE status = 'active' AND name = ?").get(proposed.section);
        if (!match) {
 ...
 */
export function detectContradiction(proposedText) {
    const db = getDb();
    const existing = db.prepare("SELECT name, content FROM shared_standards WHERE status = 'active'").all();
    const negationPairs = [
        ["always", "never"],
        ["must", "must not"],
        ["required", "forbidden"],
    ...
    for (const std of existing) {
        ...
        for (const [positive, negative] of negationPairs) {
            if ((proposedLower.includes(positive) && stdLower.includes(negative)) ||
 ...
 */
function checkAgentConsensus(section, proposedText, currentAgent) {
    const db = getDb();
  ...
  `).all(Date.now() - 7 * 24 * 60 * 60 * 1000, currentAgent);
    const agentsWithSimilar = new Set();
    ...
    for (const ref of recentReflections) {
        try {
            const updates = JSON.parse(ref.proposed_updates);
 ...
 */
function textSimilarity(a, b, bWords) {
    ...
    if (aWords.size === 0 || targetWords.size === 0)
        return 0;
    let overlap = 0;
 ...
 */
export function extractPatternFromPeaks(projectId) {
    const db = getDb();
  ...
  `).all(projectId);
    ...
    for (const [, cluster] of clusters) {
        if (cluster.contents.length < 3)
            continue;
        // Check if a standard already exists for this topic
        const existingStd = db.prepare("SELECT 1 FROM shared_standards WHERE status = 'active' AND name LIKE ?").get(`%${cluster.heading.slice(0, 30)}%`);
        if (existingStd)
            continue;
 ...
 */
export function consolidateStandards() {
    const db = getDb();
    ...
    const remaining = db.prepare("SELECT * FROM shared_standards WHERE status = 'active' ORDER BY priority ASC").all();
    let currentTotal = remaining.reduce((sum, s) => sum + s.content.length, 0);
    for (const std of remaining) {
        if (currentTotal <= MAX_STANDARDS_BUDGET)
            break;
        db.prepare("UPDATE shared_standards SET status = 'disabled', updated_at = ? WHERE id = ?")
            .run(now, std.id);
...
function inferCategory(section) {
    ...
    if (lower.includes("name") || lower.includes("naming") || lower.includes("convention"))
        return "naming";
    if (lower.includes("tool") || lower.includes("prefer") || lower.includes("library"))
        return "tool_preference";
    if (lower.includes("workflow") || lower.includes("process") || lower.includes("pipeline"))
        return "workflow";
    return "coding_norm";
}
//# sourceMappingURL=sop-engine.js.map