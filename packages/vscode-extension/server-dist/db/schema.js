...
export function initSchema(db) {
  ...
  `);
    ...
    if (!exists) {
        const now = Date.now();
        db.prepare("INSERT INTO channels (id, name, description, status, type, project_id, workplace_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NULL, NULL, ?, ?)").run("general", "General", "Default group chat", "active", "public", now, now);
    }
}
