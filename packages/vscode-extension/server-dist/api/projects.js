...
function countProjectMemories(projectId) {
    const db = getDb();
...
function listActiveProjectAgents(projectId) {
    const db = getDb();
    return db.prepare(`
...
function syncProjectAgentContexts(projectId) {
    for (const agent of listActiveProjectAgents(projectId)) {
        syncAgentWorkspaceContext(agent.agent_name);
    }
}
function announceProjectAgentUpdate(projectId, action, subjectAgent) {
    const db = getDb();
    const project = db.prepare("SELECT name FROM projects WHERE id = ?").get(projectId);
    const channel = db.prepare("SELECT id, type FROM channels WHERE project_id = ? AND workplace_id IS NULL ORDER BY created_at ASC LIMIT 1").get(projectId);
    if (!project || !channel)
        return;
...
function createWorkplace(project, name, kind = "derived") {
    const db = getDb();
    const now = Date.now();
  ...
  `).run(id, project.id, name, slug, directory, kind, now, now);
    ...
    `).run(channelId, `${project.name}/${name}`, `Workplace channel for ${project.name} - ${name}`, project.id, id, now, now);
        broadcast({
    ...
    return db.prepare("SELECT * FROM workplaces WHERE id = ?").get(id);
}
...
router.get("/by-agent/:name", (req, res) => {
    const db = getDb();
...
router.post("/", (req, res) => {
    const { directory, name, description, tech_stack, config } = req.body;
    ...
    const existingDir = db.prepare("SELECT name FROM projects WHERE directory = ?").get(directory);
    if (existingDir) {
  ...
  `).run(id, projectName, slug, description || "", techStackJson, configJson, directory, now, now);
    ...
    const channelExists = db.prepare("SELECT 1 FROM channels WHERE id = ?").get(channelId);
    if (!channelExists) {
        db.prepare("INSERT INTO channels (id, name, description, status, type, project_id, created_at, updated_at) VALUES (?, ?, ?, 'active', 'public', ?, ?, ?)").run(channelId, projectName, `Project channel for ${projectName}`, id, now, now);
        broadcast({
            type: "channel:created",
...
router.get("/:id/workplaces", (req, res) => {
    const db = getDb();
...
router.post("/:id/workplaces", (req, res) => {
    const db = getDb();
    const project = db.prepare("SELECT * FROM projects WHERE id = ?").get(req.params.id);
    if (!project) {
        res.status(404).json({ error: "project not found" });
    ...
    const existing = db.prepare("SELECT * FROM workplaces WHERE project_id = ? AND slug = ?").get(project.id, slug);
    if (existing) {
        res.json(existing);
        return;
    }
...
router.put("/:id", (req, res) => {
    ...
    const { name, description, tech_stack, config } = req.body;
    const now = Date.now();
    db.prepare(`
    UPDATE projects SET name = ?, description = ?, tech_stack = ?, config = ?, updated_at = ? WHERE id = ?
  `).run(name || existing.name, description !== undefined ? description : existing.description, tech_stack ? JSON.stringify(tech_stack) : existing.tech_stack, config ? JSON.stringify(config) : existing.config, now, existing.id);
...
router.post("/:id/pause", (req, res) => {
    ...
    if (existing.status !== "active") {
        res.status(400).json({ error: "only active projects can be paused" });
        return;
    }
    const now = Date.now();
...
router.post("/:id/resume", (req, res) => {
    ...
    if (existing.status !== "paused") {
        res.status(400).json({ error: "only paused projects can be resumed" });
        return;
    }
    const now = Date.now();
...
router.delete("/:id", (req, res) => {
    ...
    const affectedAgents = db.prepare("SELECT agent_name FROM project_agents WHERE project_id = ?").all(existing.id);
...
router.post("/:id/agents", (req, res) => {
    const { agent_name, role_in_project, assignment_type } = req.body;
    if (!agent_name) {
...
router.delete("/:id/agents/:name", (req, res) => {
    const db = getDb();
    const projectId = typeof req.params.id === "string" ? req.params.id : String(req.params.id);
...
router.put("/:id/agents/:name/workplace", (req, res) => {
    const db = getDb();
    const projectId = typeof req.params.id === "string" ? req.params.id : String(req.params.id);
    const agentName = typeof req.params.name === "string" ? req.params.name : String(req.params.name);
    const { workplace_id } = req.body;
    const assignment = db.prepare("SELECT * FROM project_agents WHERE project_id = ? AND agent_name = ? AND status = 'active'").get(projectId, agentName);
    if (!assignment) {
        res.status(404).json({ error: "active project assignment not found" });
        return;
    }
    let nextWorkplaceId = null;
    if (workplace_id) {
        const workplace = db.prepare("SELECT * FROM workplaces WHERE id = ? AND project_id = ?").get(workplace_id, projectId);
        if (!workplace) {
    ...
    db.prepare("UPDATE project_agents SET active_workplace_id = ? WHERE project_id = ? AND agent_name = ?").run(nextWorkplaceId, projectId, agentName);
    syncAgentWorkspaceContext(agentName);
    broadcast({
...
router.get("/:id/workplaces/:workplaceId/context", (req, res) => {
    const db = getDb();
    const project = db.prepare("SELECT * FROM projects WHERE id = ?").get(req.params.id);
    if (!project) {
        res.status(404).json({ error: "project not found" });
    ...
    const workplace = db.prepare("SELECT * FROM workplaces WHERE id = ? AND project_id = ?").get(req.params.workplaceId, project.id);
    if (!workplace) {
...
router.post("/:id/memory/share", (req, res) => {
    const { target_project_id, entry_ids } = req.body;
  ...
  `);
    ...
    const tx = db.transaction(() => {
        for (const entryId of entry_ids) {
            const row = selectStmt.get(entryId, req.params.id);
            if (!row) {
                skipped++;
                continue;
            }
            ...
            const dupCheck = db.prepare("SELECT 1 FROM memory_entries WHERE content_hash = ? AND scope_type = 'project' AND scope_id = ? AND status != 'archived'").get(row.content_hash, target_project_id);
            if (dupCheck) {
                skipped++;
                continue;
            }
            const now = Date.now();
            insertStmt.run(newId, row.agent_name, row.category, row.source_file, row.heading, row.content, row.content_hash, row.importance, row.emotional_weight, now, now, 0, "[]", 1.0, 0.3, 0.0, 1.0, row.status === "permanent" ? "permanent" : "active", row.linked_ids, target_project_id, row.embedding, "project", target_project_id);
            copied++;
        }
    });
    tx();
