...
import { clearAgentMetadataKeys, DEFAULT_PROVIDER, getAgentRuntimeConfig, mergeAgentMetadata, getProvider, } from "../agent-runtime.js";
...
function createClaudeInstructions(name, role) {
    return `# Agent: ${name}
Role: ${role}
...
function createCodexInstructions(name, role) {
    return `# Agent: ${name}
Role: ${role}
...
function createCodexConfigToml(name, role) {
...
function createWorkspace(name, role, provider) {
...
async function killAgentProcesses(name) {
    try {
...
function getClaudeSettingsPath(agentDir) {
    return path.join(agentDir, ".claude", "settings.local.json");
}
function computeClaudeBridgeFingerprint(agentDir) {
...
function buildClaudeLaunchSpec(agentDir, claudePath, agentName, options) {
    ...
    const sessionId = forceNewSession ? randomUUID() : config.claudeSessionId;
...
async function getTmuxPanePid(session) {
    try {
...
export async function startVerifiedClaudeSession(agentDir, claudePath, agentName, options) {
...
router.post("/register", (req, res) => {
    ...
    const existing = db
        ...
        .get(name);
    if (existing) {
        db.prepare("UPDATE agents SET status = 'online', role = ?, last_heartbeat = ? WHERE name = ?").run(role || "", now, name);
        broadcast({ type: "agent:status", data: { name, status: "online", role: role || "", provider } });
        res.json({ id: existing.id, name, status: "online" });
        return;
    }
    const id = randomUUID();
    db.prepare("INSERT INTO agents (id, name, provider, role, status, last_heartbeat, registered_at) VALUES (?, ?, ?, ?, 'online', ?, ?)").run(id, name, provider, role || "", now, now);
    broadcast({ type: "agent:status", data: { name, status: "online", role: role || "", provider } });
    res.json({ id, name, status: "online" });
});
// POST /agents/create - Create workspace + register + optionally wake
router.post("/create", async (req, res) => {
    const { name, role, wake, provider, instructions, requested_by, permissions } = req.body;
    ...
    const agentProvider = provider === "codex" ? "codex" : DEFAULT_PROVIDER;
    ...
    if (existing) {
        ...
        agentId = randomUUID();
        db.prepare("INSERT INTO agents (id, name, provider, role, status, last_heartbeat, registered_at) VALUES (?, ?, ?, ?, 'offline', ?, ?)").run(agentId, name, agentProvider, agentRole, now, now);
    }
    syncAgentWorkspaceContext(name);
...
router.get("/", (req, res) => {
    ...
    const result = agents.map((a) => {
        const p = getProviderByType(a.provider);
        return {
            ...a,
...
router.get("/:name/terminal", (_req, res) => {
    const { name } = _req.params;
    const content = getProviderFor(name).getTerminalContent(name);
    res.json({ name, content });
});
// POST /agents/:name/terminal/input - Send input to agent's runtime
router.post("/:name/terminal/input", async (req, res) => {
    ...
    try {
        await getProviderFor(name).sendInput(name, input, type);
        res.json({ ok: true });
    }
    catch (err) {
...
router.put("/:name", (req, res) => {
    ...
    db.prepare("UPDATE agents SET role = ?, status = ? WHERE name = ?").run(newRole, newStatus, name);
...
router.get("/:name/worklog", (req, res) => {
    const { name } = req.params;
    ...
    try {
        ...
        res.status(500).json({ error: `Failed to read worklog: ${err.message}` });
    }
});
// PUT /agents/:name/worklog - Save agent's worklog
router.put("/:name/worklog", (req, res) => {
    const { name } = req.params;
    ...
    try {
        ...
        res.status(500).json({ error: `Failed to save worklog: ${err.message}` });
    }
});
// --- Agent config: model and effort ---
const ALLOWED_CLAUDE_MODELS = ["sonnet", "opus"];
const ALLOWED_EFFORTS = ["medium", "high", "max"];
const ALLOWED_APPROVAL_POLICIES = ["untrusted", "on-request", "never"];
const ALLOWED_SANDBOX_MODES = ["read-only", "workspace-write", "danger-full-access"];
...
export function getAgentProjectContext(agentName) {
    const db = getDb();
  ...
  `).get(agentName);
    if (!assignment)
        return null;
    const techStack = JSON.parse(assignment.tech_stack);
    const standards = db
...
export function buildClaudeCmd(agentDir, claudePath, agentName) {
 ...
 */
function extractWorkspaceInfo(projectContext) {
...
function buildRuntimeProjectGuard(projectContext) {
    const lines = projectContext.split("\n");
    const relevant = [
...
router.put("/:name/instructions", (req, res) => {
    const { name } = req.params;
    const { instructions, requested_by } = req.body;
...
router.put("/:name/config", async (req, res) => {
    const { name } = req.params;
    const { model, effort, approval_policy, sandbox_mode, requested_by, restart } = req.body;
    const provider = getProvider(name);
    ...
    // Validate model
    if (provider === "claude" && model !== undefined && !ALLOWED_CLAUDE_MODELS.includes(model)) {
        res.status(400).json({ error: `Invalid model. Allowed: ${ALLOWED_CLAUDE_MODELS.join(", ")}` });
        return;
    }
    // Validate effort
    if (effort !== undefined && !ALLOWED_EFFORTS.includes(effort)) {
    ...
    const db = getDb();
    const existing = db.prepare("SELECT metadata FROM agents WHERE name = ?").get(name);
    if (!existing) {
        res.status(404).json({ error: "agent not found" });
        return;
    }
    ...
    if (effort !== undefined)
        meta.effort = effort;
    if (approval_policy !== undefined)
        meta.approvalPolicy = approval_policy;
    if (sandbox_mode !== undefined)
        meta.sandboxMode = sandbox_mode;
    ...
    if (restart) {
        try {
            await getProviderFor(name).restart(name);
        }
        catch {
...
router.get("/:name/runtime-status", (req, res) => {
    const { name } = req.params;
    const provider = getProvider(name);
    const p = getProviderFor(name);
    const config = getAgentRuntimeConfig(name);
    res.json({
        name,
        provider,
        runtimeState: p.getState(name),
        contextPercent: p.getContextPercent(name),
        config,
    });
});
async function authorOnlyAction(req, res, action) {
...
router.post("/:name/interrupt", async (req, res) => {
    const { name } = req.params;
    await authorOnlyAction(req, res, async () => {
        return getProviderFor(name).interrupt(name);
    });
});
router.post("/:name/resume", async (req, res) => {
    const { name } = req.params;
    await authorOnlyAction(req, res, async () => {
        return getProviderFor(name).resume(name);
    });
});
router.post("/:name/reset-session", async (req, res) => {
    const { name } = req.params;
    await authorOnlyAction(req, res, async () => {
        clearAgentMetadataKeys(name, ["threadId"]);
        await getProviderFor(name).restart(name, { resetSession: true });
        return true;
    });
});
