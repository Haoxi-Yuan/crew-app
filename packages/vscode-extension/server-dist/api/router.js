...
router.get("/mentions/:agentName", (req, res) => {
    const { agentName } = req.params;
    const db = getDb();
    const pending = db
        .prepare(`SELECT pm.id, pm.message_id, pm.agent_name, pm.created_at,
...
router.post("/mentions/:id/ack", (req, res) => {
    ...
    const db = getDb();
    db.prepare("UPDATE pending_mentions SET acknowledged = 1 WHERE id = ?").run(mentionId);
    res.json({ ok: true });
});
function findClaudePath() {
    return findBinary("claude");
}
async function startAgentRuntime(name, options) {
...
async function stopAgentRuntime(name) {
    const provider = getProvider(name);
    if (provider === "codex") {
...
router.post("/agents/:name/system-stop", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: `Failed to stop agent: ${err.message}` });
    }
});
router.post("/agents/:name/system-restart", async (req, res) => {
    const name = typeof req.params.name === "string" ? req.params.name : String(req.params.name);
    try {
        ...
        res.status(500).json({ error: `Failed to restart agent: ${err.message}` });
    }
});
// Wake all agents
router.post("/wake-all", (_req, res) => {
    const agentsDir = path.join(PROJECT_ROOT, "agents");
    ...
    for (const name of agentNames) {
        ...
        if (provider === "codex") {
            startCodexAgent(name)
                ...
                remaining--;
                if (remaining === 0)
                    res.json({ ok: true, launched, skipped });
            })
                .catch(() => {
                remaining--;
                if (remaining === 0)
...
router.get("/system/pick-directory", (_req, res) => {
...
router.post("/system/detect-tech-stack", (req, res) => {
