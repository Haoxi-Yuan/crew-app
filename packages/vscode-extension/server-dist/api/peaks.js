...
const router = Router();
function persistPeakDecisionMemory(row, chosenOption, options, chosenIndex, note, decidedBy, now) {
    const db = getDb();
  ...
  `).run(memId, row.agent_name, `Peak decision: ${chosenOption.label}`, content, contentHash, now, now, scope.projectId, scope.scopeType, scope.scopeId);
    return memId;
}
function buildSettlement(row, chosenOption, optionIndex, note, decidedBy, memoryId, channelId, messageId) {
    return {
        peak_id: row.id,
...
function resolvePeakChannel(row) {
    const db = getDb();
    if (row.project_id) {
      ...
      LIMIT 1
    `).get(row.project_id, row.agent_name);
        if (assignment?.active_workplace_id) {
            const workplaceChannel = db.prepare("SELECT id, type FROM channels WHERE workplace_id = ? ORDER BY created_at ASC LIMIT 1").get(assignment.active_workplace_id);
            if (workplaceChannel)
                return workplaceChannel;
        }
        const projectChannel = db.prepare("SELECT id, type FROM channels WHERE project_id = ? AND workplace_id IS NULL ORDER BY created_at ASC LIMIT 1").get(row.project_id);
        if (projectChannel)
            return projectChannel;
    }
    return { id: "general", type: "public" };
}
function queueSettlementMessage(row, chosenOption, note, decidedBy, memoryId) {
    const db = getDb();
  ...
  `).run(channel.id, content, JSON.stringify([row.agent_name]), now, JSON.stringify({ peak_id: row.id, memory_id: memoryId || null }));
...
router.post("/", (req, res) => {
    const { agent_name, project_id, peak_type, context, options = [], agent_lean, default_option = 0, timeout_seconds = 300, } = req.body;
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
  `).run(id, agent_name, project_id || null, peak_type, context, JSON.stringify(options), agent_lean || null, default_option, Math.min(Math.max(timeout_seconds, 30), 1800), now);
    const peak = {
        id,
        agent_name,
...
router.get("/", (req, res) => {
    const { status, agent_name, project_id, limit = "20" } = req.query;
    const db = getDb();
...
router.get("/pending", (_req, res) => {
    const db = getDb();
    const rows = db.prepare("SELECT * FROM peaks WHERE status = 'pending' ORDER BY created_at ASC").all();
    res.json(rows.map(formatPeak));
});
// GET /peaks/:id - Get a single peak
router.get("/:id", (req, res) => {
    const db = getDb();
    const row = db.prepare("SELECT * FROM peaks WHERE id = ?").get(req.params.id);
    if (!row) {
        res.status(404).json({ error: "peak not found" });
        return;
    }
...
router.post("/:id/decide", (req, res) => {
    const { option_index, note, decided_by } = req.body;
...
router.post("/:id/let-agent-decide", (req, res) => {
    const db = getDb();
    const row = db.prepare("SELECT * FROM peaks WHERE id = ?").get(req.params.id);
    if (!row) {
        res.status(404).json({ error: "peak not found" });
 ...
 */
export function processExpiredPeaks() {
    const db = getDb();
    const now = Date.now();
  ...
  `).all(now);
    for (const peak of expired) {
        const options = JSON.parse(peak.options);
