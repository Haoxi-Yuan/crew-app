...
function findSessionFiles() {
    ...
    for (const projDir of projectDirs) {
        ...
        if (!fs.statSync(projPath).isDirectory())
            continue;
        const files = fs.readdirSync(projPath).filter((f) => f.endsWith(".jsonl") && !f.includes("subagent"));
...
router.post("/session", (req, res) => {
    ...
    db.prepare("INSERT INTO messages (sender_type, sender_name, content, mentions, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?)").run("system", "system", `--- Imported conversation from ${session.project} (${session.userMessages} user + ${session.assistantMessages} assistant messages) ---`, "[]", "system", now);
    const insertStmt = db.prepare("INSERT INTO messages (sender_type, sender_name, content, mentions, message_type, created_at) VALUES (?, ?, ?, ?, ?, ?)");
    const importAll = db.transaction(() => {
        for (const line of lines) {
            ...
            if (text.startsWith("<local-command") || text.startsWith("<command-name>"))
                continue;
            const msg = rec.message;
            const ts = msg?.timestamp || rec.timestamp || now;
