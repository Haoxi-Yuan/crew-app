...
router.get("/budget", (_req, res) => {
    const db = getDb();
    const rows = db.prepare("SELECT content FROM shared_standards WHERE status = 'active'").all();
...
router.post("/", (req, res) => {
    const { category, name, content, priority } = req.body;
  ...
  `).run(id, content, now);
    const row = db.prepare("SELECT * FROM shared_standards WHERE id = ?").get(id);
...
router.put("/:id", (req, res) => {
    const db = getDb();
    const existing = db.prepare("SELECT * FROM shared_standards WHERE id = ?").get(req.params.id);
    if (!existing) {
        res.status(404).json({ error: "standard not found" });
        return;
    }
    const { name, content, priority, status, change_summary } = req.body;
    ...
    if (content && content !== existing.content && targetStatus === "active") {
        const others = db.prepare("SELECT content FROM shared_standards WHERE status = 'active' AND id != ?").all(existing.id);
    ...
    WHERE id = ?
  `).run(name || existing.name, content || existing.content, priority !== undefined ? priority : existing.priority, status || existing.status, now, existing.id);
    // Record version history if content changed
    if (content && content !== existing.content) {
        const lastVersion = db.prepare("SELECT MAX(version) as v FROM standards_history WHERE standard_id = ?").get(existing.id);
    ...
    const row = db.prepare("SELECT * FROM shared_standards WHERE id = ?").get(existing.id);
...
router.get("/:id/history", (req, res) => {
    ...
    const rows = db.prepare("SELECT * FROM standards_history WHERE standard_id = ? ORDER BY version DESC").all(req.params.id);
    res.json(rows);
});
...
router.post("/:id/rollback", (req, res) => {
    ...
    const historyRow = db.prepare("SELECT * FROM standards_history WHERE standard_id = ? AND version = ?").get(req.params.id, version);
    if (!historyRow) {
