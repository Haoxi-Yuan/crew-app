...
function writeClaudeForwardEnvelope(agentName, input, messageId) {
...
function buildClaudeInboxPrompt(filePath) {
...
function describeChannel(channelId, channelType) {
    if (!channelId)
        return "chat";
    if (channelType === "dm")
        return "DM";
...
export function buildForwardedMessageInput(senderName, content, channelId, channelType, channelContext = "") {
 ...
 */
export function getRecentChannelContext(channelId, excludeMessageId) {
    try {
        const db = getDb();
        ...
        const rows = db
            .prepare("SELECT id, sender_name, content, created_at FROM messages WHERE channel_id = ? ORDER BY id DESC LIMIT ?")
            .all(channelId, limit);
        const messages = rows
 ...
 */
export async function forwardToAgent(agentName, senderName, content, channelId, channelType, messageId) {
    ...
    try {
        ...
        console.error(`[forward] failed to send to ${agentName}:`, err.message);
        return false;
    }
}
/**
 ...
 */
export async function forwardToMentionedAgents(mentions, senderName, content, messageId, channelId, channelType) {
    const forwarded = [];
    const failed = [];
 ...
 */
function updateDeliveryStatus(messageId, agentName, status) {
    try {
        const db = getDb();
        db.prepare("UPDATE pending_mentions SET delivery_status = ? WHERE message_id = ? AND agent_name = ?").run(status, messageId, agentName);
        broadcast({
            type: "message:status",
