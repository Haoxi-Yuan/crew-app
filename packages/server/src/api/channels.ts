...
import { getDb, type ChannelRow } from "../db/index.js";
import { broadcast } from "../ws/handler.js";

const router: RouterType = Router();

// Helper: parse members from DB row
function parseChannel(ch: ChannelRow) {
  return { ...ch, members: ch.members ? JSON.parse(ch.members) as string[] : null };
}
...
router.post("/dm/:agentName", (req: Request, res: Response) => {
  const { agentName } = req.params;
  const db = getDb();
  const id = `dm-${agentName}`;

  const existing = db.prepare("SELECT * FROM channels WHERE id = ?").get(id) as ChannelRow | undefined;
  if (existing) {
    // Ensure active
  ...
  const now = Date.now();
  db.prepare(
    "INSERT INTO channels (id, name, description, type, members, status, created_at, updated_at) VALUES (?, ?, ?, 'dm', ?, 'active', ?, ?)"
  ).run(id, agentName, "", JSON.stringify([agentName as string]), now, now);

  const channel = { id, name: agentName as string, description: "", type: "dm", members: [agentName as string], status: "active", created_at: now, updated_at: now };
  broadcast({ type: "channel:created", data: channel });
  res.json(channel);
});
...
router.post("/:id/members", (req: Request, res: Response) => {
  ...
  const db = getDb();
  const ch = db.prepare("SELECT * FROM channels WHERE id = ?").get(id) as ChannelRow | undefined;
  if (!ch) { res.status(404).json({ error: "channel not found" }); return; }
  if (ch.type !== "group") { res.status(400).json({ error: "can only manage members on group channels" }); return; }

  const members: string[] = ch.members ? JSON.parse(ch.members) : [];
  if (members.includes(name)) { res.json(parseChannel(ch)); return; }
...
router.delete("/:id/members/:name", (req: Request, res: Response) => {
...
router.put("/:id", (req: Request, res: Response) => {
  ...
  const now = Date.now();
  db.prepare(
    "UPDATE channels SET name = ?, description = ?, status = ?, updated_at = ? WHERE id = ?"
  ).run(
    name || existing.name,
