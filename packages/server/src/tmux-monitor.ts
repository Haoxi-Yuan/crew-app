...
const activeApprovals = new Map<string, ToolApproval>();

// In-memory store of agent tmux state
export type AgentTmuxState = "idle" | "busy" | "approval_pending" | "no_session";
const agentStates = new Map<string, AgentTmuxState>();

// In-memory store of terminal content and context percent per agent
const agentTerminalContent = new Map<string, string>();
const agentContextPercent = new Map<string, number>();

let approvalIdCounter = 0;

export function getActiveApprovals(): ToolApproval[] {
  return Array.from(activeApprovals.values());
}

export function getApprovalForAgent(agentName: string): ToolApproval | undefined {
  return activeApprovals.get(agentName);
}

export function removeApproval(agentName: string): void {
  activeApprovals.delete(agentName);
}

export function getAgentTmuxState(agentName: string): AgentTmuxState {
  return agentStates.get(agentName) || "no_session";
}

export function getAllAgentTmuxStates(): Map<string, AgentTmuxState> {
  return new Map(agentStates);
}

export function getAgentContextPercent(agentName: string): number {
  return agentContextPercent.get(agentName) || 0;
}

export function getAllAgentContextPercents(): Map<string, number> {
  return new Map(agentContextPercent);
}

export function getAgentTerminalContent(agentName: string): string {
 ...
 */
function parseContextPercent(paneContent: string): number | null {
...
/**
 * Detect if pane content contains any Claude Code approval prompt.
 * Returns the type of prompt detected, or null if none found.
 *
 * Claude Code prompt types:
 * - "tool_use": MCP tool calls ("Do you want to proceed?")
 * - "file_edit": File edit approval ("Do you want to make this edit")
 * - "file_create": File creation ("Do you want to create this file")
 * - "bash": Bash command ("Do you want to execute")
 * - "plan_execute": Plan execution ("Would you like to proceed?")
 * - "session_feedback": Session rating ("How is Claude doing this session?")
 * - "mcp_setup": MCP server activation ("Enter to confirm" + "MCP server")
 */
type ApprovalPromptType = "tool_use" | "file_edit" | "file_create" | "bash" | "plan_execute" | "session_feedback" | "mcp_setup";

function detectApprovalType(paneContent: string): ApprovalPromptType | null {
  if (paneContent.includes("Do you want to proceed?")) return "tool_use";
  if (paneContent.includes("Do you want to make this edit")) return "file_edit";
  if (paneContent.includes("Do you want to create")) return "file_create";
  if (paneContent.includes("Do you want to execute")) return "bash";
  if (paneContent.includes("Would you like to proceed?")) return "plan_execute";
  if (paneContent.includes("How is Claude doing this session?")) return "session_feedback";
  if (paneContent.includes("Enter to confirm") && paneContent.includes("MCP server")) return "mcp_setup";
  return null;
}

function parseApprovalPrompt(paneContent: string, agentName: string): ToolApproval | null {
  ...
  if (promptType === "tool_use") {
    const toolMatch = paneContent.match(
      /(?:Tool use\s*\n?\s*)([\w-]+)\s*-\s*([\w_]+)\(([^)]*(?:\([^)]*\))*[^)]*)\)\s*\(MCP\)/s
    );
    if (toolMatch) {
  ...
  const anchorIdx = Math.max(
    paneContent.lastIndexOf("Do you want to proceed?"),
    paneContent.lastIndexOf("Do you want to make this edit"),
    paneContent.lastIndexOf("Do you want to create"),
    paneContent.lastIndexOf("Do you want to execute"),
    paneContent.lastIndexOf("Would you like to proceed?"),
    paneContent.lastIndexOf("How is Claude doing this session?"),
    paneContent.lastIndexOf("Enter to confirm"),
    paneContent.lastIndexOf("MCP documentation")
  );
  ...
  if (options.length === 0) {
    if (promptType === "mcp_setup") {
      options.push(
        { key: "1", label: "Use this and all future MCP servers in this project" },
        { key: "2", label: "Use this MCP server" },
        { key: "3", label: "Continue without using this MCP server" }
      );
    } else if (promptType === "session_feedback") {
      options.push(
        { key: "1", label: "Bad" },
      ...
      options.push(
        { key: "1", label: "Yes, clear context and auto-accept edits" },
        { key: "2", label: "Yes, auto-accept edits" },
        { key: "3", label: "Yes, manually approve edits" },
        { key: "4", label: "Edit plan" }
      );
    } else {
      options.push(
        { key: "1", label: "Yes" },
 ...
 */
function detectState(paneContent: string): AgentTmuxState {
...
export async function sendApprovalResponse(
  ...
  try {
    ...
    console.error(`[tmux-monitor] Failed to send approval response to ${agentName}:`, (err as Error).message);
    return false;
  }
}
  ...
  for (const sessionName of sessionNames) {
    ...
    try {
      const agent = db
        ...
        .get(agentName) as { status: string } | undefined;
...
const lastCycleTime = new Map<string, number>();
 ...
 */
async function checkAutoCycle(): Promise<void> {
  for (const [agentName, percent] of agentContextPercent) {
    if (percent < SESSION_AUTO_CYCLE_CONTEXT_PERCENT) continue;

    // Enforce cooldown
    const lastCycle = lastCycleTime.get(agentName) || 0;
    if (Date.now() - lastCycle < SESSION_AUTO_CYCLE_COOLDOWN_MS) continue;

    // Don't cycle agents that are in approval_pending state
    const state = agentStates.get(agentName);
    if (state === "approval_pending") continue;

    console.error(
      `[tmux-monitor] Auto-cycling agent "${agentName}" (context: ${percent}%)`
    );
    ...
    try {
      ...
      console.error(
        `[tmux-monitor] Failed to cycle "${agentName}":`,
...
async function cycleAgentSession(agentName: string): Promise<void> {
  const sessionName = `crew-${agentName}`;
  ...
  try {
    ...
    const existing = db.prepare(
      "SELECT id FROM messages WHERE channel_id = 'general' AND sender_name = 'crew-manager' AND message_type = 'auto-cycle' AND metadata = ?"
    ).get(JSON.stringify({ agent: agentName })) as { id: number } | undefined;
    ...
    if (existing) {
      ...
      const result = db.prepare(
        "INSERT INTO messages (channel_id, sender_type, sender_name, content, mentions, message_type, created_at, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
      ).run("general", "system", "crew-manager", content, "[]", "auto-cycle", now, JSON.stringify({ agent: agentName }));
      broadcast({
        type: "message:new",
...
export function startTmuxMonitor(): void {
...
export function stopTmuxMonitor(): void {
  if (monitorInterval) {
